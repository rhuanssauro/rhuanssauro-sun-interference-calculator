...
Teleport NOC operators and field VSAT engineers in South America. They sit a dark console during equinox season and need a yes/no plus UTC window for a named bird, a remote site, and a dish size — not a marketing explainer. Default example site is Macaé, Rio de Janeiro, Brazil.
...
Voice: short NOC sentences. Band, slot, UTC. No SaaS cheer.
...
- Verdict language matches owner tools: impacted / not impacted / not in view / out of season, plus UTC start–end when a window exists.
- Content is visible with motion off. Motion only reports state.

## Accessibility & Inclusion

WCAG 2.2 AA. Body text ≥ 7:1 on the surface. Status is never color-only (text + shape). `prefers-reduced-motion` disables motion. Keyboard: chips focusable, drop zone activatable with Enter/Space, form labels associated. Default language English with Portuguese place names (Macaé) untranslated.
